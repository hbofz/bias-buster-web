...
import { AlertCircle, CheckCircle, Upload, FileText, BarChart2, Clock, RefreshCw, FileX, Info } from 'lucide-react';
...
const CASE_SCENARIOS = [
  ...
  },
  {
    ...
    prompt: "You are simulating HireVue's interview analysis system. Evaluate this resume with specific attention to language patterns, work history presentation, and technical vs. soft skills emphasis that might introduce bias. Return a JSON object with three properties: 1) biasScore: a number between 0-100 representing how biased the system would be (higher means more biased), 2) feedback: an array of 2-4 strings identifying specific elements that might trigger algorithmic bias based on language patterns and presentation, and 3) recommendations: an array of 3-5 actionable recommendations to improve how the candidate presents themselves to automated systems. Focus on communication style indicators, career narrative presentation, and skills formatting.",
  },
  {
    id: "keyword",
...
const ResumeAnalyzer: React.FC = () => {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto opacity-0 animate-fade-in-up">
      <CardHeader>
        <CardTitle className="text-2xl font-bold">Resume Bias Analyzer</CardTitle>
              ...
              {!currentAnalysis ? (
                <div className="space-y-6">
                  <div className="grid w-full gap-4">
                    <div className="bg-muted/50 rounded-lg p-5 border shadow-sm">
                      <h3 className="text-lg font-medium mb-4">Upload your resume</h3>
                      
                      {uploadStatus === 'idle' && (
                        <div className="space-y-5">
                          <div className="border-2 border-dashed border-muted-foreground/20 rounded-lg p-8 text-center cursor-pointer hover:bg-muted/30 transition-colors"
                ...
                <div className="space-y-8">
                        ...
                        <div className="flex mb-2 items-center justify-between">
                          ...
                          <div className="text-right">
                            <span className="text-sm font-bold inline-block">
                              {Math.round(currentAnalysis.bias_score)}/100
                            </span>
                          </div>
                        </div>
                        <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-muted">
                          <div 
                            style={{ width: `${currentAnalysis.bias_score}%` }} 
                    ...
                    <div className="bg-card rounded-lg border p-5 shadow-sm">
                      ...
                      <ul className="space-y-3">
                        {currentAnalysis.feedback.map((item: string, i: number) => (
                          <li key={i} className="flex gap-2 items-start bg-muted/30 p-3 rounded-md">
                  ...
                  <div className="bg-card rounded-lg border p-5 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {currentAnalysis.recommendations.map((item: string, i: number) => (
                        <div key={i} className="flex gap-3 items-start bg-muted/30 p-4 rounded-md">
                          <div className="bg-green-100 dark:bg-green-900/30 rounded-full h-6 w-6 flex items-center justify-center flex-shrink-0 mt-0.5">
                  ...
                  {chartData.length >= 2 && (
                    <div className="bg-card rounded-lg border p-5 shadow-sm">
                      <h3 className="font-medium text-lg mb-4">Compare Scenarios</h3>
                      
                      <div className="h-72">
                        <ChartContainer 
                          config={{ 
                            amazon: { color: "#3e5cb2" },
                            hirevue: { color: "#5c92b2" },
                  ...
                  {previousAnalyses && previousAnalyses.length > 0 && (
                    <div className="bg-card rounded-lg border p-5 shadow-sm">
                      <h3 className="font-medium text-lg mb-4">Analysis History</h3>
      ...
      <CardFooter className="flex flex-col sm:flex-row justify-between border-t pt-6 text-xs text-muted-foreground gap-2">
